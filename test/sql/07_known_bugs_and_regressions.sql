...
-- both n and m, with dst-alias WHERE/RETURN fields silently resolving to
-- null/unenforced. Now every label-matching `n` candidate is actually
-- traversed: default (non-prefixed) tables batch through a new pure-C
-- adjacency-list BFS (igraph_match_traverse_multi_internal, pg_igraph.c)
-- that loads the edge table once and walks every candidate in memory;
-- prefixed tables batch the same candidates into a single recursive CTE
-- seeded from all of them at once. resolve_match_alias_id (igraph_exec.c)
-- was simplified to resolve each row's own (src_id, dst_id) pair instead
-- of one global anchor. Both rows below now correctly show the real
-- U1->U2 edge (n=U1, m=U2) instead of the old n==m self-pairs with a
-- null m.name. Kept here rather than moved into REGRESSION ANCHORS, to
-- avoid shifting every node id generated later in this file (same reason
-- the NumBug fixture below stayed put after task #21).
-- ============================================================
...
-- task #17 (continued): unanchored MATCH with more than one label-matching
-- `n` candidate, each with its own distinct outgoing edge -- proves the
-- batched traversal (igraph_match_traverse_multi_internal) walks every
-- candidate, not just the first/only one.
SELECT graph_add_node('UnanchoredMulti') AS um1 \gset
SELECT graph_add_node('UnanchoredMulti') AS um2 \gset
SELECT graph_add_node('UnanchoredMulti') AS um3 \gset
SELECT graph_add_node('UnanchoredMulti') AS um4 \gset
SELECT graph_add_edge(:um1, :um2, 'mlink');
SELECT graph_add_edge(:um3, :um4, 'mlink');
SELECT graph_set_property(:um1, 'name', 2::smallint, str_to_bytea('M1'));
SELECT graph_set_property(:um2, 'name', 2::smallint, str_to_bytea('M2'));
